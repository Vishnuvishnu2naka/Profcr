...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[           
            InlineKeyboardButton('📢 𝙐𝙥𝙙𝙖𝙩𝙚 📢', url=f'https://t.me/{SUPPORT_CHAT}')
            ],[
            InlineKeyboardButton('♻️ 𝐇𝐞𝐥𝐩 ♻️', url=f"https://t.me/{temp.U_NAME}?start=help")
            ]]
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton("♻️ 𝐓𝐫𝐲 𝐀𝐠𝐚𝐢𝐧 ", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton("♻️ 𝐓𝐫𝐲 𝐀𝐠𝐚𝐢𝐧 ", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
